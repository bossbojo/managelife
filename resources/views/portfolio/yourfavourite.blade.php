  ...
  <div class="col-md-9">
    <div class="panel panel-default  border-radiusbox" style="background-color:#333; border-color:#222; ">
      <div class="panel-heading border-radius-top-20px2" style="background-color:#cc9900; border-color:#222;">
        <label for="" style="font-size:20px;"><i class="glyphicon glyphicon-star"></i> Your favorite</label>
      </div>
      <div class="panel-body" style="border-color:#222;">
        ...
        $userport = Favorite::all();
        foreach ($userport as $userports) {
          $yourlike = explode(',', $userports->favorite);
          $check = 0;
          for ($i=0; $i < sizeof($yourlike); $i++) {
            if($yourlike[$i] == Auth::user()->id ){
              $check = 1;
            }
          }
          if($check == 1){
            $user = User::find($userports->id);
