...
use App\Feedback;
  ...
  $index = Indexportfolio::find($UserView->id);
 ?>
...
@if (!Auth::guest())
<?php
$check = Favorite::find($UserView->id);
$numtof = 0;
$usernow = 0;
if($check != ''){
 ...
 ?>
  <div style="z-index:999; position:absolute; bottom:0px; padding-left:30px;">
    <div style="background-color:rgba(0, 0, 0, 0.35); border-radius:5px;  padding:5px;">
      <?php if($usernow == 0){ ?>
        <a id="en" href="#" class="btn btn-default" onclick="enfavorite({{ $UserView->id }},{{ Auth::user()->id }})" >
         <i style="color:rgba(166, 255, 0, 0.95); font-size:20px;" class="glyphicon glyphicon-star"></i> favorite (<span id="enf">{{ $numtof }}</span>)
        </a>
        <a id="de" href="#" class="btn btn-default" onclick="defavorite({{ $UserView->id }},{{ Auth::user()->id }})" style="display:none;">
         <i style="color:rgba(166, 255, 0, 0.95); font-size:20px;" class="glyphicon glyphicon-star"></i> favorite (<span id="def">{{ $numtof }}</span>)
        </a>
      <?php }else{ ?>
        <a id="en" href="#" class="btn btn-default" onclick="enfavorite({{ $UserView->id }},{{ Auth::user()->id }})" style="display:none;">
         <i style="color:rgba(166, 255, 0, 0.95); font-size:20px;" class="glyphicon glyphicon-star"></i> favorite (<span id="enf">{{ $numtof }}</span>)
        </a>
        <a id="de" href="#" class="btn btn-default" onclick="defavorite({{ $UserView->id }},{{ Auth::user()->id }})" >
         <i style="color:rgba(166, 255, 0, 0.95); font-size:20px;" class="glyphicon glyphicon-star"></i> favorite (<span id="def">{{ $numtof }}</span>)
        </a>
      <?php } ?>
      <a href="#" class="btn btn-default" onclick="openfeedback()">
        <i class="glyphicon glyphicon-comment"></i> Comment for this portfolio (0)
      </a>
    </div>
  </div>
  <div class="modal" id="openfeedback">
    <div class="col-md-6 col-md-offset-3" style="background-color:#444; padding:10px; border-radius:5px;">
      <table>
        <tbody>
           <tr>
             <td>
               <?php
                ...
                ?>
              <img class="img-circle" style="{{ Auth::user()->filter }}" src="{{ $img }}" alt="" width="200px">
             </td>
             <td style="padding:10px;width:100%;">
               <div class="form-group">
                <label for="comment" style="color:#aaa;">Feedback:</label>
                <textarea class="form-control" rows="5" style="width:100%;" id="comment" placeholder="Please enter your feedback..."></textarea>
              </div>
              <strong style="color:red;" id="alert"></strong>
              <div class="" align="right">
                <a href="#" class="btn btn-danger" onclick="closefeedback()">Cancel</a>
                <a href="#" class="btn btn-success" onclick="feedback({{ $UserView->id }},{{ Auth::user()->id }})">Submit</a>
              </div>
             </td>
           </tr>
        </tbody>
...
<script type="text/javascript">
  ...
  function closefeedback() {
    $('#openfeedback').fadeOut();
  }
  function feedback(id,user_id) {
    console.log(id);
    var data2 = { Id: id ,
                  user_Id : user_id,
                  feedback : $('#comment').val()
                }
    var url2 = "{{ url('/savefeedback') }}"; // the script where you handle the form input.
    $.ajax({
     type: "get",
     url: url2,
     data: data2 , // serializes the form's elements.
     success: function(result) {
       ...
       if(result == 'success'){
         ...
         $('#alert').html('Failed uplaod your feedback');
       }
     },
     error: function(xhr,textStatus){ alert(textStatus);}
   });
  }
  function enfavorite(id,user_id) {
    console.log(id);
    var data2 = { Id: id ,
                  user_Id : user_id,
                }
    var url2 = "{{ url('/enfavorite') }}"; // the script where you handle the form input.
    $.ajax({
     type: "get",
     url: url2,
     data: data2 , // serializes the form's elements.
  ...
  function defavorite(id,user_id) {
    console.log(id);
    var data2 = { Id: id ,
                  user_Id : user_id,
                }
    var url2 = "{{ url('/defavorite') }}"; // the script where you handle the form input.
    $.ajax({
     type: "get",
     url: url2,
     data: data2 , // serializes the form's elements.
...
.btn-circle.btn-xl {
  ...
  <div style="z-index:999; position:absolute; bottom:20px; right:50px;">
    <div style="background-color:rgba(0, 0, 0, 0.5); padding:5px; border-radius:5px;">
      <a href="{{ url('/presentUserfull').'/'.$UserView->id }}" class="btn btn-warning btn-circle btn-xl">
...
<div class="se-pre-con"></div>
  <div canvas="container"  style=" background-image:url('<?=  asset($index->background) ?>'); ">
    ...
    <script type="text/javascript">
      ...
      <?php
      $to;
      $nowlogin = Indexportfolio::find($UserView->id);
                    ...
                    _.each(items, function (node, i) {
                      if(str[i]%6==0){
                        this.grid.addWidget($('<div id="'+arr_index[i]+'"'+' style="overflow:hidden;" ><div style="overflow:hidden;" class="grid-stack-item-content " >' +
                                              ...
                                              '<img src="{{ asset($UserView->avatar) }}" style="{{ $UserView->filter }}height:100%;width:100%;object-fit: cover;">'+
                                                '<div class="w3-container w3-padding-8" style="background-color:'+datato[arr_index[i]+""+9]+'; color:'+datato[arr_index[i]+""+7]+';">'+
                                                  '<h4><b>{{$UserView->name}}</b></h4>'+
                        ...
                        this.grid.addWidget($('<div id="'+arr_index[i]+'"'+'><div style="overflow:hidden   ;" class="grid-stack-item-content ">' +
                                                '<div class="w3-card-4 w3-margin w3-white" style="background-color:'+datato[arr_index[i]+""+9]+';" >'+
                                                  '<video width="100%" controls>'+
                                                    '<source src="{{ asset('') }}'+datato[arr_index[i]+""+2]+'" type="video/mp4">'+
                                                  '</video>'+
                                                  '<div class="w3-container w3-padding-8"  style="background-color:'+datato[arr_index[i]+""+9]+'; color:'+datato[arr_index[i]+""+7]+';">'+
                      ...
                      if(str[i]%6==2){
                        ...
                        this.grid.addWidget($('<div id="'+arr_index[i]+'"'+'><div style="overflow:hidden   ;"  class="grid-stack-item-content ">' +
                                                '    <div class="w3-card-4 w3-margin w3-white" style="background-color:'+datato[arr_index[i]+""+9]+';" >'+
                                                    '<img src="{{ asset('') }}'+img+'" style="height:100%;width:100%;object-fit: cover;">'+
                                                    '  <div class="w3-container w3-padding-8"  style="background-color:'+datato[arr_index[i]+""+9]+'; color:'+datato[arr_index[i]+""+7]+';">'+
                                                      '  <h4><b>'+datato[arr_index[i]+""+4]+'</b></h4>'+
                                                        '<h5>'+datato[arr_index[i]+""+5]+', <span class="w3-opacity">'+datato[arr_index[i]+""+6]+'</span></h5>'+
                                                  '    </div>'+
                      ...
                      if(str[i]%6==4){
                        ...
                        this.grid.addWidget($('<div id="'+arr_index[i]+'"'+' style="overflow:visible;"><div style="overflow:hidden   ;" class="parent grid-stack-item-content ">' +
                                '    <div class="w3-card-4 w3-margin w3-white">'+
                                    '<img src="{{ asset('') }}'+img2+'" style="width:100%;object-fit: cover;">'+
                                  '  </div>'+
                              '</div></div>'),
                            node.x, node.y, node.width, node.height);
                        }
                ...
                function addNewWidget(ih) {
                    ...
                    if(ih == 0){ // profile
                      ...
                      '<div class="w3-card-4 w3-margin w3-white">'+
                        ...
                        '</div>'+
                        '<img src="{{ asset($UserView->avatar) }}" style="height:100%;width:100%;object-fit: cover;">'+
                          '<div class="w3-container w3-padding-8">'+
                            '<h3><b>{{$UserView->fristname}}&nbsp;&nbsp;{{$UserView->lastname}} ({{$UserView->name}})</b></h3>'+
